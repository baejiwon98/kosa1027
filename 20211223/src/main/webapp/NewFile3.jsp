...
<%= str3.indexOf("ing") %> <br />
str3에서 index가 4이후에 있는 st는 몇번째 index인가 <%= str3.indexOf("st", 4) %> <br />
<%= str3.indexOf("st",4) %> <br />
str5에서 "프로그래밍"의 index는 : <%= str5.indexOf("프로그래밍") %><br />
<%= str5.indexOf("프로그래밍") %><br />

str3에서 6번째 index부터 출력 : <%= str3.substring(6) %><br />
<%= str3.substring(6) %><br />
str 4에서 index가 4부터 5글자만 가져오기 : <%= str4.substring(4, 9) %><br />
<%= str4.substring(4, 9) %><br />
str5에서 4번째 index부터 출력 : <%= str5.substring(4) %><br />
<%= str5.substring(4) %><br />

str3을 공백문자를 구분자로 문자열을 잘라 변수에 저장하고 자른 문자열을 저장한 변수를 이용하여 값을 출력 :
<br />
<%
	String []s = str3.split(" ");
...
<%
	...
	for(int i=0; i <st.length; i++) {%>
		st[<%=i %>] = <%=st[i] %> <br />		
<% } %>
str6에 있는 문자열을 각각의 단어로 잘라 저장한 후 저장된 변수의 값을 출력하시오.<br />
...
<%
	...
	out.print("strMatrix1의 배열의 크기 : " + strMatrix1[0].length + "<br />");
	out.print("strMatrix1의 배열의 크기 : " + strMatrix1[1].length + "<br />");
